...
/* Safer Replacement Policy - SHARP secure hierarchical policy*/
template <bool sharersAware>
class SHARPReplPolicy : public ReplPolicy {
    protected:
        uint64_t timestamp; // incremented on each access
        uint64_t* array;
        uint32_t numLines;
    ...
    public:
        explicit SHARPReplPolicy(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
            array = gm_calloc<uint64_t>(numLines);
        }
    ...
    private:
        inline uint64_t score(uint32_t id, uint32_t srcId) { //higher is least evictable
            //array[id] < timestamp always, so this prioritizes by:
            // (1) valid (if not valid, it's 0)
            // (2) sharers, and
            // (3) timestamp
            return (sharersAware? cc->numSharers(id) : 0)*timestamp + array[id]*cc->isValid(id) + cc->isSharer(id,srcId) ;
            //return array[id]*cc->isValid(id);
        }
};
