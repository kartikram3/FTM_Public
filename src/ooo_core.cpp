...
extern Cache* llc_ptr;
extern Cache* llc_ptr_1;
extern Cache** llc_ptrs;
extern int llc_banks;
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
            ...
            case UOP_LOAD:
                {
                    ...
                    if (zinfo->isCEASER){
                       if ( (float)count >= (float)(zinfo->refreshCount) ){
                         count = 0;
                         //info("Invoking process refresh from %lx", (uint64_t)llc_ptr);
                         //llc_ptr->randomo();
                         //((Cache *)(zinfo->llc_ptr))->processRefresh(0);
                         //llc_ptr_1->processRefresh();
                         for (uint32_t i=0; (int)i<zinfo->llc_banks; i++){
                            zinfo->llc_ptrs[i]->processRefresh(i);
                         }
                       }
                    }
                    ...
                    if (addr != ((Address)-1L)) {
                        ...
                        count++;
                        reqSatisfiedCycle = reqSatisfiedCycle + 10;
                        #if defined GPG_ATTACK
                            uint64_t lat = reqSatisfiedCycle - dispatchCycle; 
                            if ( addr ==  0x7f73e7480000 ){
                               fprintf(stderr,"Mul latency is %lu, cycle is %lu\n", lat, reqSatisfiedCycle); 
                            }

                            if ( addr == 0x7f73e7481000 ){
                               fprintf(stderr,"Sqr latency is %lu, cycle is %lu\n", lat, reqSatisfiedCycle); 
                            }

                           //if ( addr == 0x7f73df480000){
                           //    fprintf(stderr,"Accessed addr 0x7f73df480000\n");
                           //}

                        #elif defined PDFTOPS_ATTACK

                            uint64_t lat = reqSatisfiedCycle - dispatchCycle; 
                            if ( addr ==  0x7f73e7480000 ){
                               info("opSetFont latency is %lu\n", lat); 
                            }

                            if ( addr == 0x7f73e7481000 ){
                               info("opShowSpaceText latency is %lu\n", lat); 
                            }
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
        ...
        //info("The wrongPathAddr is %p\n", (void *)wrongPathAddr);
        uint64_t reqCycle = fetchCycle;
        for (uint32_t i = 0; i < 5*64/lineSize; i++) {
            #if 0 //defined GPG_ATTACK
               //modify attacked address so that it can target the 
               //appropriate address
               if ((wrongPathAddr  == 0x49a350) || (wrongPathAddr == 0x49a4d0)){
               //if (((wrongPathAddr & 0xfffffffffffff000) == 0x497000) || ((wrongPathAddr & 0xfffffffffffff000) == 0x49a000)){
                   fprintf(stderr, "multiply-reduce step in zsim\n");
                   //jfprintf(stderr, "The multiply-reduce cycle is %lx\n", curCycle);
                   wrongPathAddr = 0x7f73e7480000; //set0, not set no. 0
               } 
               if ((wrongPathAddr == 0x499d90) || (wrongPathAddr==0x499ea0)){
                   fprintf(stderr, "sqr step in zsim\n");
                   wrongPathAddr = 0x7f73e7481000; //set1, not set no. 1
               //    //fprintf(stderr, "The sqr cycle is %lx\n", curCycle);

               } 
               //modify spy process so it can target the chosen cache line
            #endif

            //#if defined PDFTOPS_ATTACK
            //   if (wrongPathAddr == 0x7ffff7a3ae00){
            //       fprintf(stderr,"Function opSetFont in zsim");
            //   } 
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += lineSize) {
        ...
        #if defined GPG_ATTACK
               newFetchAddr = fetchAddr; 
               if ((fetchAddr  == 0x49a350) || (fetchAddr == 0x49a4d0)){
               //if (((fetchAddr & 0xfffffffffffff000) == 0x497000) || ((fetchAddr & 0xfffffffffffff000) == 0x49a000)){
                   fprintf(stderr, "multiply-reduce step in zsim\n");
                   newFetchAddr = 0x7f73e7480000;
                   //fprintf(stderr, "The multiply-reduce cycle is %lx\n", curCycle);
               } 

               if ((fetchAddr == 0x499d90) || (fetchAddr==0x499ea0)){
                   fprintf(stderr, "sqr step in zsim\n");
                   newFetchAddr = 0x7f73e7481000;
                   //fprintf(stderr, "The sqr cycle is %lx\n", curCycle);

               } 

        uint64_t fetchLat = l1i->load(newFetchAddr, curCycle) - curCycle;
        #elif defined PDFTOPS_ATTACK
               newFetchAddr = fetchAddr; 
               if (zinfo->load_1 == true){
                   info("opSetFont step in zsim, address is %lx \n", fetchAddr);
                   newFetchAddr = 0x7f73e7480000;
                   zinfo->load_1 = false;
               } 

               if (zinfo->load_2 == true){
                   info("opShowSpaceText step in zsim, address is %lx\n", fetchAddr);
                   newFetchAddr = 0x7f73e7481000;
                   zinfo->load_2 = false;
               } 
        uint64_t fetchLat = l1i->load(newFetchAddr, curCycle) - curCycle;
